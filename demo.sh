...
declcfg-inline-bundles --prune-non-head-objects index 
...
##
yq eval ".spec.replaces=\"demo-operator.v${VERSION}\"" -i config/manifests/bases/demo-operator.clusterserviceversion.yaml
...
##
## 1. Download catalog image
##
mkdir index-dl
opm alpha render ${IMAGE_TAG_BASE}-index:latest > index-dl/index.yaml
...
##
opm alpha render ${IMAGE_TAG_BASE}-bundle:v${VERSION} -o yaml >> index/index.yaml
yq eval-all "(select(.schema==\"olm.bundle\" and .name==\"demo-operator.v0.2.0\") | .properties[] | select(.type==\"olm.channel\" and .value.name==\"stable\") | .value.replaces)=\"demo-operator.v${VERSION}\"" -i index/index.yaml
declcfg-inline-bundles --prune-non-head-objects index
opm alpha validate index
...
export IMG=${IMAGE_TAG_BASE}:v${VERSION}
make docker-build docker-push
yq eval 'del(.spec.replaces)' -i config/manifests/bases/demo-operator.clusterserviceversion.yaml
...
##
yq eval-all '(select(.name=="demo-operator.v0.3.0").properties[] | select(.type=="olm.channel").value.replaces)="demo-operator.v0.2.0"' -i index/index.yaml
